            ...
            <table class="table table-striped result <?php echo $role ?>-matches" data-role="<?php echo $role ?>" >
                ...
                <tr>
                    ...
                    <?php if($role == \MissionNext\lib\Constants::ROLE_CANDIDATE): ?>
                        <th class="sortable"><?php echo __('Username', \MissionNext\lib\Constants::TEXT_DOMAIN) ?></th>
                ...
                <tbody>
                <?php foreach($groups as $group_name => $folderItems): ?>
...
<script>
    ...
    jQuery(document).on('click', 'table.result tr td.match-highlight div', function(){
        ...
        spinner.show();
        showMatchHighlight( jQuery(this).attr('data-item_profile'), jQuery(this).attr('data-item_results'), jQuery(this).data('user-role'), function(data){
    ...
    function showMatchHighlight(profile, results, userrole, success, error){
        jQuery.ajax({
            type: "POST",
