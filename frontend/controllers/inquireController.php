...
class inquireController extends AbstractLayoutController {
    ...
    public function inquire(){
        ...
        $mail_service->send($organization['email'], __("Job Inquiry", Constants::TEXT_DOMAIN), UserLib::replaceTokens($text_message, $this->user, $organization, $this->domain, $job), '', array(), false);

        foreach($affiliates as $affiliate){

            if($affiliate['status'] == 'approved'){
                $mail_service->send($affiliate['agency_profile']['email'], __("Job Inquiry", Constants::TEXT_DOMAIN), UserLib::replaceTokens($text_message, $this->user, $affiliate['agency_profile'], $this->domain, $job), '', array(), false);
            }
    ...
    public function cancel(){
        ...
        $mail_service->send($organization['email'], sprintf(__("%s Inquiry Canceled", Constants::TEXT_DOMAIN), ucfirst(getCustomTranslation(Constants::ROLE_JOB))), UserLib::replaceTokens($text_message, $this->user, $organization, $this->domain, $job), '', array(), false);

        foreach($affiliates as $affiliate){

            if($affiliate['status'] == 'approved'){
                $mail_service->send($affiliate['agency_profile']['email'], sprintf(__("%s Inquiry Canceled", Constants::TEXT_DOMAIN), ucfirst(getCustomTranslation(Constants::ROLE_JOB))), UserLib::replaceTokens($text_message, $this->user, $affiliate['agency_profile'], $this->domain, $job), '', array(), false);
            }
    ...
    public function cancelByAgency(){
        ...
        $mail_service->send($organization['email'], sprintf(__("%s Inquiry Canceled", Constants::TEXT_DOMAIN), ucfirst(getCustomTranslation(Constants::ROLE_JOB))), UserLib::replaceTokens($text_message, $this->user, $organization, $this->domain, $job), '', array(), false);
        $mail_service->send($candidate['email'], sprintf(__("%s Inquiry Canceled", Constants::TEXT_DOMAIN), ucfirst(getCustomTranslation(Constants::ROLE_JOB))), UserLib::replaceTokens($text_message, $this->user, $candidate, $this->domain, $job), '', array(), false);
